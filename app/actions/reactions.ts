...
export async function toggleChapterReaction(
  ...
  if (!user) {
    return { success: false, error: "برای ثبت واکنش باید وارد شوید" };
  }
  if (!isValidReactionEmoji(emoji)) {
    return { success: false, error: "واکنش نامعتبر است" };
  }
  ...
  if (existing && existing.emoji === emoji) {
    ...
    await prisma.chapterReaction.update({
      where: { chapterId_userId: { chapterId, userId: user.id } },
      data: { emoji },
    });
    userReaction = emoji;
    ...
    await prisma.chapterReaction.create({
      data: { chapterId, userId: user.id, emoji },
    });
    userReaction = emoji;
  }
  ...
  return {
    success: true,
    summary: grouped.map((g) => ({ emoji: g.emoji, count: g._count._all })),
    userReaction,
  };
}

export async function getChapterReactionSummary(
  chapterId: string,
  userId: string | null
): Promise<{ summary: ReactionSummary[]; userReaction: string | null }> {
  const [grouped, mine] = await Promise.all([
    ...
    userId
      ? prisma.chapterReaction.findUnique({
