...
export function TeamManager({ initialStaff }: { initialStaff: StaffMember[] }) {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    const result = await addPublisherStaff({ telegramUsername: username, role, canUpload });
    if (!result.success) {
    ...
    <div className="space-y-4">
      <form onSubmit={handleSubmit} className="flex flex-wrap items-end gap-2 rounded-md border border-border bg-surface p-4">
        <div className="space-y-1">
          ...
          <input id="staff-username" value={username} onChange={(e) => setUsername(e.target.value)} required className="rounded-md border border-border bg-background px-3 py-2 text-sm text-text-main" />
        </div>
        <div className="space-y-1">
          <label className="text-xs text-text-muted" htmlFor="staff-role">نقش</label>
          <select id="staff-role" value={role} onChange={(e) => setRole(e.target.value as StaffRole)} className="rounded-md border border-border bg-background px-3 py-2 text-sm text-text-main">
            {ROLES.map((r) => (
      ...
      <div className="divide-y divide-border rounded-md border border-border">
        {staff.map((s) => (
          <div key={s.id} className="flex items-center justify-between px-4 py-3">
            <p className="text-sm text-text-main">{s.user.firstName} {s.user.username ? `— @${s.user.username}` : ""} · {s.role}{s.canUpload ? "" : " (بدون مجوز آپلود)"}</p>
            <button onClick={() => handleRemove(s.id)} disabled={pending} className="rounded-md border border-red-400 px-2 py-1 text-xs text-red-400 disabled:opacity-50">حذف</button>
          </div>
        ))}
