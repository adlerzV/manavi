  ...
  return (
    <div className="flex items-center gap-1 rounded-full bg-black/40 p-1">
      <button
        ...
        aria-label="حالت اسکرول عمودی"
        className={`flex h-7 w-7 items-center justify-center rounded-full ${
          mode === "VERTICAL" ? "bg-white/20 text-white" : "text-white/50"
        }`}
      >
      ...
      <button
        ...
        aria-label="حالت صفحه‌بندی افقی"
        className={`flex h-7 w-7 items-center justify-center rounded-full ${
          mode === "HORIZONTAL" ? "bg-white/20 text-white" : "text-white/50"
        }`}
      >
        <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
